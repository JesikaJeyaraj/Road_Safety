...
function createComplaintPDF(complaint, imagePath, outputPath) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const contractor = complaint.contractorDetails || {};
      const hazard = complaint.hazardResults || { level: 'Medium', score: 50 };
...
Estimated Hazard Severity: ${hazard.level} (Score: ${hazard.score}/100)

--- CONTRACTOR ACCOUNTABILITY & BUDGET LEDGER ---
Responsible Contractor: ${contractor.contractorName || 'NHAI Local Contractor JV'}
Highway Corridor: ${contractor.highwayName || 'National Highway Local Zone'}
Allocated Maintenance Budget: ${contractor.allocatedBudgetWords || '₹85.0 Crores'} (INR ${contractor.allocatedBudgetRupees || '85,00,00,000'})
Contract Maintenance Period: ${contractor.maintenancePeriod || '2024 - 2027'} (Status: ${contractor.status || 'Active'})

A verification protocol has been run against the uploaded media. The system verified GPS geo-tags embedded in the file, ensuring time and location consistency. Our classification model confirmed the presence of the road defect. Please hold the contracted engineering partner accountable and dispatch the maintenance team to audit this location and take corrective actions at the earliest.`;

      doc.font('Helvetica').fontSize(9.5).text(bodyText, {
        align: 'justify',
        lineGap: 3
      });
      ...
      const verificationHash = complaint.metadataResults?.details?.sha256 || complaint.sha256 || 'unknown';
      const truncatedHash = typeof verificationHash === 'string' ? `${verificationHash.substring(0, 16)}...` : 'unknown';
      doc.fillColor('#777777')
         .fontSize(8)
         .font('Helvetica-Oblique')
         .text(`This document is dynamically generated by the NHAI Safe-Drive Automated Portal. System Originality Verification: ${complaint.metadataResults?.status || 'UNKNOWN'} (Hash: ${truncatedHash})`, 50, footerY, {
           width: 495,
           align: 'center'
         });
